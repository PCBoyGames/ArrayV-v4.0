...
*/
final public class InPlaceOptimizedSmartSafeStalinSortInserts extends Sort {

    int segmentcount;

    public InPlaceOptimizedSmartSafeStalinSortInserts(ArrayVisualizer arrayVisualizer) {
        super(arrayVisualizer);

        this.setSortListName("In-Place Optimized Smart Safe Stalin (Inserts)");
        this.setRunAllSortsName("In-Place Optimized Smart Safe Stalin Sort (Inserts)");
        this.setRunSortName("In-Place Optimized Smart Safe Stalinsort (Inserts)");
        ...
        while (!pass) {
            ...
            for (int i = 1; i + 1 < currentLength; i++) {
                if (Reads.compareIndices(array, collection - 1, i, 0.5, true) <= 0) {
                    if (collection != i) Writes.insert(array, i, collection, 0.01, true, nochan = false);
                    pass = false;
                    collection++;
                }
            }
