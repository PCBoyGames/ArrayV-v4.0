...
public class InPlaceOptimizedSafeStalinSortSwaps extends Sort {
    public InPlaceOptimizedSafeStalinSortSwaps(ArrayVisualizer arrayVisualizer) {
        ...
        this.setSortListName("In-Place Optimized Safe Stalin (Swaps)");
        this.setRunAllSortsName("In-Place Optimized Safe Stalin Sort (Swaps)");
        this.setRunSortName("In-Place Optimized Safe Stalinsort (Swaps)");
        ...
        for (int i=start; i<end; i++) {
            for (int j=i+1; j<end; j++) {
                if (Reads.compareValues(array[j], array[i]) >= 0) {
                    if (++i != j) Writes.swap(array, j, i, 5, true, false);
                }
            }
            runs++;
        }
