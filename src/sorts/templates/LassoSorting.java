...
import sorts.insert.InRunBinaryInsertionSort;
...
public abstract class LassoSorting extends Sort {
    ...
    protected InRunBinaryInsertionSort binserter;
    ...
    protected void rotateMerge(int[] array, int start, int mid, int end) {
		if(start==mid||mid==end)
			return;
		if(end-start < 4) {
			this.binserter.irbinsert(array, start, end, 1, true, false);
			return;
		}
    ...
    public void common(int[] array, int start, int end) {
    	this.binserter = new InRunBinaryInsertionSort(arrayVisualizer);
    	int len = end-start;
    	if(len <= 4) {
    		...
    		this.binserter.irbinsert(array, start, end, 0.5, true, false);
    		return;
    	}
    	...
    	this.binserter.irbinsert(array, BufLoc, BufLoc+BufSize, 0.5, true, false);
    	this.rotateMerge(array, start, BufLoc, BufLoc+BufSize);
		this.binserter.irbinsert(KeySpace, 0, BufSize, 0.5, true, true);
