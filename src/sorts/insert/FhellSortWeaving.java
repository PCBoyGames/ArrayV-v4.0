...
*/
public class FhellSortWeaving extends MadhouseTools {
    public FhellSortWeaving(ArrayVisualizer arrayVisualizer) {
        super(arrayVisualizer);
        this.setSortListName("Fhell (Weaving)");
        this.setRunAllSortsName("Fhell Sort With Weaving");
        this.setRunSortName("Fhellsort Weaving");
    ...
    protected boolean isPow3(int n) {
        if (n < 1) return false;
        while (n % 3 == 0) n /= 3;
        return n == 1;
    }
    ...
    public void runSort(int[] array, int currentLength, int bucketCount) {
        ...
        for (int i = currentLength / 2; i >= 1; i--) if (currentLength % i == 0) {
            ...
            int x = i;
            int d = isPow3(l) && isPow3(i) ? 2 : 3;
            for (; x < l; x *= d);
            for (; x >= i; x /= d) shellPass(array, currentLength, x, false);
            l = i;
        }
    }
}