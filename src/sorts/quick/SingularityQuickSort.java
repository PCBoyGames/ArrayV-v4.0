    ...
    protected void singularityQuick(int[] array, int start, int offset, int end, int depth, int rep) {
        ...
        if (end - start > 7 && depth < depthlimit && rep < 4) {
            ...
            if (left < end) {
                ...
                if (lsmall && left - start > 0) {
                    Writes.recursion();
                    if (end - 4 <= left || left <= start + 4) singularityQuick(array, start, start, left, depth + 1, rep + 1);
                    else singularityQuick(array, start, start, left, depth + 1, 0);
                }
                if (end - (left + 1) > 0) {
                    Writes.recursion();
