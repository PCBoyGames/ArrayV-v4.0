...
*/
public class OptimizedWhySort extends Sort {
    ...
    protected void bubble(int[] array, int start, int end, int dir) {
        ...
        while (lastswap >= start + 1) {
            int j = start;
            for (int i = start; i <= lastswap; i++) if (Reads.compareIndices(array, i - 1, i, 0.001, true) == dir) Writes.swap(array, i - 1, j = i, 0.001, true, false);
            lastswap = j;
        }
    }
    ...
    protected void pop18(int[] array, int start, int end, int dir) {
        if (end - start < 4) pop19(array, start, end, dir);
        else {
    ...
    public void runSort(int[] array, int currentLength, int bucketCount) {
        if (currentLength < 4) pop19(array, 1, currentLength, 1);
        else {
