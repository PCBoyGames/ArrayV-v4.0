...
public final class ReimuSort extends Sort {
    ...
    protected void rotate(int[] array, int[] buf, int start, int[] cnts, int[] blks, int bLen) {
        ...
        if (i != j)
            Writes.arraycopy(array, i, array, j, blks[2] * bLen, 1, true, false);
        j -= cnts[1];
        Writes.arraycopy(buf, bLen, array, j, cnts[1], 1, true, false);
        i -= blks[1] * bLen;
        j -= blks[1] * bLen;
        if (i != j)
            Writes.arraycopy(array, i, array, j, blks[1] * bLen, 1, true, false);
        j -= cnts[0];
    ...
    protected int[] partition(int[] array, int[] buf, int[] tags, int a, int b, int bLen, int piv) {
        ...
        for (int i = a; i < b; i++) {
            ...
            if (cmp < 0) {
                Writes.write(buf, cnts[0]++, array[i], 0.5, false, true);
                if (cnts[0] >= bLen) {
                    Writes.arraycopy(buf, 0, array, t, bLen, 1.0, true, false);
                    cnts[0] = 0;
                    blks[0]++;
                    t += bLen;
                    Writes.write(tags, bCnt++, 0 * bLen, 0, false, true);
                }
            } else if (cmp == 0) {
                Writes.write(buf, bLen + cnts[1]++, array[i], 0.5, false, true);
                if (cnts[1] >= bLen) {
                    Writes.arraycopy(buf, bLen, array, t, bLen, 1.0, true, false);
                    cnts[1] = 0;
                    blks[1]++;
                    t += bLen;
                    Writes.write(tags, bCnt++, 1 * bLen, 0, false, true);
                }
            } else {
                Writes.write(buf, 2 * bLen + cnts[2]++, array[i], 0.5, false, true);
                if (cnts[2] >= bLen) {
                    Writes.arraycopy(buf, 2 * bLen, array, t, bLen, 1.0, true, false);
                    cnts[2] = 0;
                    blks[2]++;
                    t += bLen;
                    Writes.write(tags, bCnt++, 2 * bLen, 0, false, true);
                }
            }
        }
    ...
    protected void sortHelper(int[] array, int[] buf, int[] tags, int l, int r, int bLen) {
        ...
        while (queue.size() > 0) {
            ...
            if (bad) {
                int n = b - a;
                pIdx = medianOfMedians(array, a, n - (1 - n % 2));
            } else
                pIdx = medianOf9(array, a, b);
            ...
            if (rLen == 0) {
                if (lLen > this.threshold)
                    queue.offer(new Partition(a, pr[0], eqLen < lLen / 8));
                else
                    insertSort(array, a, pr[0]);
                continue;
            }
            if (lLen == 0) {
                if (rLen > this.threshold)
                    queue.offer(new Partition(pr[1], b, eqLen < rLen / 8));
                else
                    insertSort(array, pr[1], b);
                continue;
            }
            bad = rLen / 8 > lLen || lLen / 8 > rLen;
            if (lLen > this.threshold)
                queue.offer(new Partition(a, pr[0], bad));
            else
                insertSort(array, a, pr[0]);
            if (rLen > this.threshold)
                queue.offer(new Partition(pr[1], b, bad));
            else
                insertSort(array, pr[1], b);
        }
    }
