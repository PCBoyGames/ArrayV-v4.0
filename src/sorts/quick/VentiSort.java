...
public final class VentiSort extends Sort {
    ...
    protected int[] partition(int[] array, int[] buf, int a, int b, int bLen, int piv, boolean bias,
        ...
        if (min > 0) {
            ...
            for (int i = 0, j = 0, k = 0; i < min; i++) { // set bit buffers
                while (!this.pivCmp(array[a + j * bLen + wLen], piv, bias))
                    j++;
                while (this.pivCmp(array[a + k * bLen + wLen], piv, bias))
                    k++;
    ...
    protected void sortHelper(int[] array, int[] buf, int left, int right, int bLen, boolean badImbalance) {
        ...
        while (queue.size() > 0) {
            ...
            int[] pr = partition(array, buf, curStart, curEnd, bLen, array[pIdx], bias, bad);
            if ((pr[1] & 0x1) != 0)
                continue;
            int m = pr[0];
            ...
            if (lLen == 0) {
                bias = !bias;
                pr = partition(array, buf, curStart, curEnd, bLen, array[pIdx], bias, bad);
                m = pr[0];
