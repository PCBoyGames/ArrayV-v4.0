...
*/
public class PlaygroundSort extends Sort {
    ...
    protected void chase(int[] array, int item, int target) {
        Writes.multiSwap(array, item, target - item > 0 ? target - 1 : target + 1, 0.005, true, false);
    }

    protected void quit(int[] array, int bound, int item) {
        Writes.multiSwap(array, item, bound - 1, 0.005, true, false);
    }
